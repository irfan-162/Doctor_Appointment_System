...
export default function BillList() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const res = await fetch(`http://localhost:3001/api/doctor/billList`, {
          method: "GET",
          headers: {
  ...
  const updatePaymentStatus = async (id) => {
    ...
    const res = await fetch(`http://localhost:3001/api/doctor/payCheck?appID=${id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${token}`, // send token
  ...
  return (
    <div className="pl-app">
      <div className="pl-header">
        <h1>Bill Pending <span>List</span></h1>
        {!loading && (
                  ...
                  <td className="td-actions">
                    <div className="action-group">
                      <button
                        ...
                        onClick={() => updatePaymentStatus(p.id)}
                      >
                        PAID
                      </button>
                    </div>
